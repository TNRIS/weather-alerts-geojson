...
function flattenProperties(featureCollection, preserveStyle) {
  ...
  newFeatureCollection.features.forEach(function (feature) {
    feature.properties = flattenObjectArray(feature.properties, preserveStyle);
  });

  return newFeatureCollection;
}

function flattenObjectArray(arr, preserveStyle) {
  var flattened = {};

  arr.forEach(function (obj, i) {
    Object.keys(obj).forEach(function (key) {
      var flatKey = String(key) + String(i +1);
      flattened[flatKey] = obj[key];
    });
  });

  if (preserveStyle) {
    flattened['fill'] = flattened['fill1'];
  }
